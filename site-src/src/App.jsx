...
function Stars() {
  ...
  const stars = Array.from({ length: count }, (_, i) => ({
    ...
    size: Math.random()*2+0.5, dur: 2+Math.random()*4, delay: Math.random()*4
  }))
  return (
...
function Marquee() {
  ...
  return (
    <div style={{ overflow: 'hidden', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', padding: '12px 0', background: 'var(--surface)', margin: '48px 0' }}>
      <motion.div
        animate={{ x: ['0%', '-50%'] }}
        transition={{ duration: 25, repeat: Infinity, ease: 'linear' }}
        style={{ display: 'flex', gap: 40, whiteSpace: 'nowrap', fontFamily: "'Share Tech Mono', monospace", fontSize: 11, letterSpacing: '0.1em', color: 'var(--muted)' }}>
        {doubled.map((t, i) => (
...
function SpecCard({ icon, label, value, index }) {
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: 18, transition: 'border-color 0.2s' }}>
...
function LoreItem({ name, status, desc, index }) {
  return (
    <motion.div
      ...
      <div style={{ position: 'absolute', left: -20, top: 5, width: 8, height: 8, borderRadius: '50%', background: status === 'alive' || status === 'windows' ? statusColor[status] : 'var(--bg)', border: `2px solid ${statusColor[status]}` }} />
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
        <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 12 }}>{name}</span>
        <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 9, color: statusColor[status], padding: '2px 7px', border: `1px solid ${statusColor[status]}`, borderRadius: 3 }}>{statusLabel[status]}</span>
      </div>
...
function SectionLabel({ children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
      <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 11, color: 'var(--accent)', letterSpacing: '0.2em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>// {children}</span>
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth, margin: '0 auto', padding }}>
        ...
        <motion.section style={{ minHeight: '100svh', display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingTop: isMobile ? 60 : 80, paddingBottom: 40, y: heroY }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            ...
            {[
              ...
              <span key={b.text} style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: isMobile ? 10 : 11, padding: '4px 10px', borderRadius: 4, border: `1px solid ${b.color}`, color: b.color, background: `${b.color}12`, letterSpacing: '0.04em' }}>{b.text}</span>
            ))}
          </motion.div>
          ...
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            ...
            <br /><span style={{ color: 'var(--win)', fontSize: isMobile ? '0.38em' : '0.42em', letterSpacing: '0.05em' }}>Windows Edition</span>
          </motion.h1>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            ...
            <a href="https://github.com/moonlightOS-Meow/moonlightos-meow/tree/meow-ntlite" target="_blank"
              style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: isMobile ? 12 : 13, padding: isMobile ? '10px 18px' : '12px 24px', borderRadius: 6, background: 'var(--accent)', border: '1px solid var(--accent)', color: '#0a0a0f', fontWeight: 700, textDecoration: 'none', letterSpacing: '0.05em' }}>
              → View on GitHub
            </a>
            <a href="https://github.com/moonlightOS-Meow/moonlightos-meow/actions" target="_blank"
              style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: isMobile ? 12 : 13, padding: isMobile ? '10px 18px' : '12px 24px', borderRadius: 6, background: 'transparent', border: '1px solid var(--border)', color: 'var(--muted)', textDecoration: 'none', letterSpacing: '0.05em' }}>
              ⚙ Build Pipeline
            </a>
        ...
        <section style={{ marginBottom: 72 }}>
          ...
          <motion.div
            ...
            style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: isMobile ? 20 : 32, position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', right: 16, top: 16, fontSize: isMobile ? 40 : 64, opacity: 0.06 }}>🐱</div>
            <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 10, color: 'var(--muted)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 4 }}>FORMAL COMPLAINTS BY LUCYFER</div>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: isMobile ? 56 : 72, color: 'var(--accent2)', lineHeight: 1, margin: '8px 0' }}>{complaint}</div>
            <p style={{ color: 'var(--muted)', fontSize: isMobile ? 13 : 14, marginTop: 8, maxWidth: 500, lineHeight: 1.7 }}>
              ...
              Her complaints have been noted, logged, and ignored. She knocked it off the table.
            </p>
        ...
        <footer style={{ borderTop: '1px solid var(--border)', padding: isMobile ? '32px 0 48px' : '40px 0 60px', textAlign: 'center' }}>
          <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: isMobile ? 26 : 32, color: 'var(--muted)', letterSpacing: '0.05em', marginBottom: 8 }}>
            <span style={{ color: 'var(--accent)' }}>moon</span>lightOS
          </div>
          <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: isMobile ? 10 : 11, color: 'var(--muted)', letterSpacing: '0.08em', lineHeight: 1.8 }}>
